...
Optimisé pour une consommation d'énergie minimale
"""
...
import psutil  # Monitoring système léger
...
class PhotoTransferService:
    def __init__(self, config_path="config.json"):
        ...
        self.photo_queue = queue.Queue(maxsize=10)  # Limite la queue pour économiser la mémoire
        self.stats = {
        ...
        # Variables d'optimisation énergétique
        self.last_activity = time.time()
        self.idle_threshold = self.config.get("system", {}).get("idle_threshold", 300)  # 5 min sans activité
        ...
        gc.enable()
        gc.set_threshold(700, 10, 10)  # Plus agressif sur le garbage collection
        ...
        while self.running:
            try:
                ...
                if camera_ok:
                    ...
                    if new_photos:
                        self.last_activity = time.time()
                        self.exit_sleep_mode()  # Sortir du mode veille si activité
                        
                        # Ajouter les nouvelles photos à la queue
                        for photo in new_photos:
                            if os.path.exists(photo):
                                try:
                                    self.photo_queue.put(photo, timeout=1)
                                    self.logger.info(f"Photo ajoutée à la queue: {photo}")
                                except queue.Full:
                                    self.logger.warning("Queue pleine, photo ignorée")
                    else:
                        # Pas de nouvelles photos, vérifier le mode veille
                        if time.time() - self.last_activity > self.idle_threshold:
                            self.enter_sleep_mode()
                
                # Optimisation des ressources système périodique
                if hasattr(self, '_last_resource_check'):
                    if time.time() - self._last_resource_check > 60:  # Toutes les minutes
                        self.check_system_resources()
                        self._last_resource_check = time.time()
                else:
                    self._last_resource_check = time.time()
                
                # Attendre avant la prochaine vérification (adaptatif selon le mode)
                sleep_time = self.config["system"]["check_interval"]
    ...
    def optimize_cpu_frequency(self):
        """Optimise la fréquence CPU selon la charge"""
        try:
            # Lecture de la charge CPU actuelle
            with open('/proc/loadavg', 'r') as f:
                load_avg = float(f.read().split()[0])
            
            self.cpu_usage_history.append(load_avg)
            if len(self.cpu_usage_history) > 10:
                self.cpu_usage_history.pop(0)
            
            avg_load = sum(self.cpu_usage_history) / len(self.cpu_usage_history)
            
            # Ajuster la fréquence selon la charge
            if avg_load < 0.3:  # Charge faible
                # Mode économie d'énergie
                subprocess.run(['sudo', 'cpufreq-set', '-g', 'powersave'], 
                             stderr=subprocess.DEVNULL, check=False)
            elif avg_load > 0.7:  # Charge élevée
                # Mode performance
                subprocess.run(['sudo', 'cpufreq-set', '-g', 'ondemand'], 
                             stderr=subprocess.DEVNULL, check=False)
                
        except Exception:
            pass  # Ignore les erreurs de gouverneur CPU
    
    def enter_sleep_mode(self):
        """Entre en mode veille économique"""
        if not self.sleep_mode:
            self.sleep_mode = True
            self.logger.info("Mode veille activé - économie d'énergie")
            
            # Réduire la fréquence de vérification
            original_interval = self.config["system"]["check_interval"]
            self.config["system"]["check_interval"] = min(original_interval * 3, 30)
    ...
    def exit_sleep_mode(self):
        ...
        if self.sleep_mode:
            self.sleep_mode = False
            self.logger.info("Mode veille désactivé - reprise normale")
            
            # Restaurer la fréquence normale
            self.config = self.load_config()  # Recharger config originale
            
            # Mode performance temporaire
            subprocess.run(['sudo', 'cpufreq-set', '-g', 'ondemand'], 
                         stderr=subprocess.DEVNULL, check=False)
    
    def check_system_resources(self):
        """Vérifie les ressources système et optimise"""
        try:
            # Vérifier la mémoire disponible
            with open('/proc/meminfo', 'r') as f:
                meminfo = f.read()
            
            # Extraire mémoire disponible
            for line in meminfo.split('\n'):
                if line.startswith('MemAvailable:'):
                    mem_available = int(line.split()[1]) * 1024  # Convertir en bytes
                    break
            else:
                return
            
            # Si moins de 100MB disponible, forcer le garbage collection
            if mem_available < 100 * 1024 * 1024:
                self.logger.warning("Mémoire faible, nettoyage forcé")
                gc.collect()
                
            # Optimiser la fréquence CPU
            self.optimize_cpu_frequency()
            
        except Exception as e:
            self.logger.debug(f"Erreur vérification ressources: {e}")
